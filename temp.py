...
limits = np.arange(100,3100,100)
stops = np.arange(100,3100,100)
distances = [10,20,50,100,150,200,300,400,500,600,700,800,900,1000]
trailing_stops = [0,10,520,0,100,200,300,400,500]

fields=["limit","stop","trailing_stop","distance","min(p.accumPL)","cumpips"]
with open(file_csv_temp, 'a') as f:
    writer = csv.writer(f)
    ...
    for stop in stops:
        for distance in distances:
            for trailing_stop in trailing_stops:
                ...
                performance.append(cumpips)
                limit_levels.append(limit)
                stop_levels.append(stop)
                distance_levels.append(distance)
                min_levels.append(min(p.accumPL))
                trailing_stop_levels.append(trailing_stop)
...
df = pd.DataFrame(list(zip(limit_levels, stop_levels,trailing_stop_levels, distance_levels,min_levels, performance)),columns=['limit','stop','trailing stop','distance','min','pips'])
df.to_csv(file_csv) 